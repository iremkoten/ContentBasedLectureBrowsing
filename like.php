...
$sql = "UPDATE VIDEOS SET VIDEOLIKE = VIDEOLIKE + 1 WHERE VIDEOID=$id";
$result = $conn->query($sql);
	
$username=$_SESSION["activeuser"];	
$sql = "INSERT INTO VIDEOLIKE (USERNAME,VIDEOID)
VALUES ('$username','$id')";
$result = $conn->query($sql);
...
?>
	...
	<div id='d3' style="position:absolute; top:180px; right:2%; z-index:1"> 
		...
		<p> <?php echo "Video Info"; echo '<pre>'; echo $_GET['videoinfo'];  echo '<pre>'; echo "Watch:"; echo $_GET['videohit']; echo '<pre>'; echo "Likes:"; echo ($_GET['videolike'])+1; if($control1!=1){echo '<a href="like.php?link=' . $_GET['link'] . '&videoname='.$_GET['videoname'].'&videoinfo='.$_GET['videoinfo'].'&videohit=' .$_GET['videohit'].'&videoid='.$_GET['videoid'].'&videolike='.$_GET['videolike'].'" >  Like</a>' ;} if($control!=1){echo '<a href="addlist.php?link=' . $_GET['link'] . '&videoname='.$_GET['videoname'].'&videoinfo='.$_GET['videoinfo'].'&videohit=' .$_GET['videohit'].'&videoid='.$_GET['videoid'].'&videolike='.$_GET['videolike'].'" >  Add my list</a>';}
